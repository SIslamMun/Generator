...
- **Tools registry**: one `configs/tools/<topic>_tools.json` per topic. The catalog is the universal grounding interface — prompts in `tool_prompts.yaml` are domain-agnostic and anchor on each tool's `description`, typed `parameters`, and real input/output `examples[]`. Two helpers for adding a new catalog:
  - `configs/tools/_TEMPLATE.json` — starter file showing all 7 parameter types + success/error examples.
  - `generator tool-lint configs/tools/<topic>_tools.json` — flags thin descriptions, invalid types, missing examples, and non-JSON outputs before you spend GPU hours generating against a broken catalog.
